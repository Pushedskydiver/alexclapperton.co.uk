...
function display_tweets(tweets) {
    ...
    jQuery.each(tweets, function(i, tweet) {
        ...
        if (tweet.text !== undefined) {
            ...
            statusHTML = '<p><svg class="svg--twitter-dims"><use xlink:href="images/icons/sprite.svg#twitter"></use></svg> <span>'+status+'</span> <a href="http://twitter.com/'+username+'/statuses/'+tweet.id_str+'">'+relative_time(tweet.created_at)+'</a></p>';
            //remove the loader
            jQuery('.tweet-loader').remove();
            //display tweet(s)
            jQuery('.twitter-feed').append(statusHTML);
        }
    });
}
