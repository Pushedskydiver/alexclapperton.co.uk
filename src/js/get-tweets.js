...
*********************************************************************/
function init_twitter_feed() {
  (function(E,q){"function"===typeof define&&define.amd?define([],q):"object"===typeof exports?module.exports=q():q()})(this,function(){function E(a){if(null===t){for(var f=a.length,b=0,k=document.getElementById(F),g="<ul class='siteFooter_tweets'>";b<f;)g+="<li>"+a[b]+"</li>",b++;k.innerHTML=g+"</ul>"}else t(a)}function q(a){return a.replace(/<b[^>]*>(.*?)<\/b>/gi,function(a,b){return b}).replace(/class="(?!(tco-hidden|tco-display|tco-ellipsis))+.*?"|data-query-source=".*?"|dir=".*?"|rel=".*?"/gi,"")}function G(a){a=a.getElementsByTagName("a");
  ...
  function handleTweets(tweets) {
      var x = tweets.length;
      var n = 0;
      var element = getElementById('siteFooter_twitterFeed');
      var html = '<ul class="siteFooter_tweets">';
      while(n < x) {
        html += '<li>' + tweets[n] + '</li>';
        n++;
      }
