...
t=document.createTextNode(d);w.appendChild(t);w.setAttribute("aria-label",d);f[c]=w}else f[c].textContent=d}d="";H?(v&&(G(a[c]),n&&G(k[c])),n&&(d+='<div class="user">'+q(k[c].innerHTML)+"</div>"),d+='<p class="siteFooter_tweet">'+'<svg class="siteFooter_twitterIcon"><title>Twitter icon</title><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="/images/icons/sprite.svg#twitter"></use></svg>'+q(a[c].innerHTML)+"</p>",A&&(d=J?d+('<p class="timePosted"><a href="'+p[c]+'">'+f[c].getAttribute("aria-label")+"</a></p>"):d+('<p class="timePosted">'+f[c].getAttribute("aria-label")+"</p>"))):(n&&(d+='<p class="user">'+k[c].textContent+"</p>"),d+='<p class="tweet">'+a[c].textContent+
